...
## Sự cố 1: `OOMKilled` — memory là lời hứa, không phải sự thật

Phần 2 giới thiệu heap; đây là nửa hợp đồng phía OS. Linux **overcommit**: khi process xin memory, kernel gật đầu một cách lạc quan — các page chỉ thành thật khi bị *chạm vào*. Nên "xin 8 GB ngon lành" và "chết khi chạm tới GB thứ 6" có thể cùng đúng.
...
- RSS leo dốc chậm qua nhiều ngày là leak của Phần 2; hình răng cưa là GC bình thường; cú nhảy bậc thang là cái DataFrame khổng lồ nọ (luật 5–10× của S02-P03 lại ra tay).
- `dmesg | grep -i oom` nói chính xác ai bị giết và vì sao — giấy chứng tử của sự cố, và câu lệnh đầu tiên đáng học thuộc.

## Sự cố 2: load average 40 trên 8 core — hàng đợi chạy
...
- Load 40, CPU 100% → bốn mươi task giành nhau tám core; mỗi đứa được ~1/5 core. Bão hoà **CPU-bound** kinh điển (câu hỏi Phần 2 có đáp án: đang tính).
- Load 40, CPU gần như *rảnh* → bốn mươi task kẹt chờ disk hoặc NFS. Dồn toa **I/O-bound** kinh điển (đáp án: đang chờ) — và thêm CPU sẽ không sửa được gì.

Riêng phân biệt đó — cùng triệu chứng, thuốc ngược nhau — đã là phần lớn giá trị của việc hiểu scheduler. Bản thân scheduler bạn cứ coi như một hàng đợi công bằng, preempt task mỗi vài mili-giây; cơ chế sâu hơn hiếm khi đổi việc bạn làm tiếp theo. (Một khoản riêng của cloud: trên instance chia sẻ, `%st` — steal — là hypervisor đem thời gian CPU của bạn cho người khác. Con instance family `t` cạn credit, cái bẫy của S04-P03, hiện hình đúng ở đây.)

## Sự cố 3: `Too many open files` — mọi thứ là file descriptor
...
## Sự cố 4: cú deploy bị treo — signal và graceful shutdown
...
Họ sự cố mà điều này giải thích: file ghi dở sau deploy, Kafka consumer xử lý lại hàng nghìn message (offset chết cùng process — at-least-once của S07-P06 gặp đúng thủ phạm), request rơi giữa đường bay. Cách sửa là một *thói quen*: *bắt SIGTERM* — ngừng nhận việc mới, làm nốt việc dở, thoát — và bảo đảm nó gọn trong thời gian ân hạn. Trong container có một bẫy con kinh điển: app chạy sau một cái shell làm PID 1 thì signal có thể không bao giờ tới nơi (`exec` process của bạn trong entrypoint). Còn khi một *zombie* hiện trong `top` (state `Z`): nó chết rồi, chỉ đang chờ process cha đến nhận exit code — bug của cha, không phải thứ giết thêm lần nữa được.

## Sự cố 5: container bị bóp — container *chính là* một process
...
- Container `OOMKilled` ở 512 MB trong khi host còn 60 GB trống — bức tường là trần *cgroup*, không phải cái máy (Sự cố 1, bản thu nhỏ).
...
Mọi thứ từ Sự cố 1–4 áp dụng *bên trong* container, với trần cgroup là những bức tường mới, gần hơn.

## Playbook triage hai phút

Năm sự cố nén thành một trình tự cố định — phần nối dài của bài debug Phần 2:
